...
$pointSQL = "SELECT users.NAME                                                            AS 
       users_name, 
       users.uid                                                             AS 
       uid, 
       Min(node_users.nid)                                                   AS 
       node_users_nid, 
       'node'                                                                AS 
       field_data_field_point_value_node_entity_type, 
       Sum(node_users__field_data_field_point_value.field_point_value_value) AS 
       node_users__field_data_field_point_value_field_point_value_v 
FROM   dr_users users 
       LEFT JOIN dr_node node_users 
              ON users.uid = node_users.uid 
       LEFT JOIN (SELECT td.*, 
                         tn.nid AS nid 
                  FROM   dr_taxonomy_term_data td 
                         LEFT JOIN dr_taxonomy_vocabulary tv 
                                ON td.vid = tv.vid 
                         LEFT JOIN dr_taxonomy_index tn 
                                ON tn.tid = td.tid 
                  WHERE  ( tv.machine_name IN ( 'activity_list_taxonomy_' ) )) 
                                                      taxonomy_term_data_node 
              ON node_users.nid = taxonomy_term_data_node.nid 
       LEFT JOIN dr_users_roles role_permission_users_roles 
              ON users.uid = role_permission_users_roles.uid 
       LEFT JOIN dr_role_permission role_permission_users_roles__role_permission 
              ON role_permission_users_roles.rid = 
                 role_permission_users_roles__role_permission.rid 
                 AND ( role_permission_users_roles__role_permission.permission = 
                       'administer blocks' 
                        OR 
role_permission_users_roles__role_permission.permission = 
'access dashboard' ) 
       LEFT JOIN dr_field_data_field_point_value 
                 node_users__field_data_field_point_value 
              ON node_users.nid = 
node_users__field_data_field_point_value.entity_id 
                 AND ( node_users__field_data_field_point_value.entity_type = 
                       'node' 
                       AND node_users__field_data_field_point_value.deleted = 
                           '0' ) 
WHERE  (( ( users.status <> '0' ) 
          AND ( role_permission_users_roles__role_permission.permission IS NULL 
              ) 
          AND ( node_users.type IN ( 'activity' ) ) )) 
GROUP  BY users_name, 
          uid, 
          field_data_field_point_value_node_entity_type 
HAVING ((( Sum(node_users__field_data_field_point_value.field_point_value_value) 
           >= 
                  '250' 
                 ))) 
ORDER  BY node_users__field_data_field_point_value_field_point_value_v DESC";
...
//How many chapters hit 2/3 or 3/3 categories: Chapter Building, Policy Change, or Training and Education
$counter;	
...
foreach ($earnersArray as $earner){
	if (array_key_exists($earner[users_name], $countArray) && ($countArray[$earner[users_name]] <= 1))
	$counter++;
}
...
echo'</a></td>
    <td>';
echo number_format((float)((count($chapterBuildersArray) / $chapter_count)*100), 2, '.', '');
echo '%</td>
</tr>
...
echo'</a></td><td>';
echo number_format((float)((count($policyChangersArray) / $chapter_count)*100), 2, '.', ''); 
echo '%</td>
</tr>
...
echo '</a></td><td>';
echo number_format((float)((count($educatorsArray) / $chapter_count)*100), 2, '.', '');
echo '%</td>
</tr>
...
echo $counter;
echo '</td><td>';
echo number_format((float)($counter / $chapter_count), 2, '.', '');
echo '% HQ</td>
</tr>
</table>
